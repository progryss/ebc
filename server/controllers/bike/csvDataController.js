...
const getCsvData = async (req, res) => {
    ...
    try {
        ...
        const csvData = await BikeCsvData.find(query)
            .skip(skip)
            .limit(limit);
        // Count only the documents that match the query
        const total = search != "" ? await BikeCsvData.countDocuments(query) : await BikeCsvData.estimatedDocumentCount();
...
const deleteMultipleRows = async (req, res) => {
    try {
        ...
        const result = await BikeCsvData.deleteMany({
            _id: { $in: rowId }
        });
...
const updateRow = async (req, res) => {
    try {
        const data = req.body;
        const updatedRow = await BikeCsvData.findByIdAndUpdate(
            data._id,
            {
                $set: {
...
const removeAllDuplicates = async (req, res) => {
    try {
        ...
        const duplicates = await BikeCsvData.aggregate([
            {
                $group: {
                    _id: {
                        make: "$make",
        ...
        for (let duplicate of duplicates) {
            ...
            await BikeCsvData.deleteMany({ _id: { $in: idsToRemove } });
